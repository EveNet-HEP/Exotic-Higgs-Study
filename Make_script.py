            ...
            for pretrain in pretrain_choice:
                for assignment, segmentation in assignment_seg_choice:
                    for dataset_size in dataset_size_choice:
                        ...
                        f.write(f" shifter --image={control['image']} python3 scripts/train.py {file_path} --ray_dir {args.ray_dir} {'--load_all' if dataset_size < 0.2 else ''} \n")
            ...
            for pretrain in pretrain_choice:
                for assignment, segmentation in assignment_seg_choice:
                    for dataset_size in dataset_size_choice:
                        ...
                        f.write(f"cd {working_dir}; ")
                        f.write(f" shifter --image={control['image']} python3 scripts/predict.py {os.path.abspath(file_path_predict)}  \n")
