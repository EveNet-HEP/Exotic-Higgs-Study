...
def plot_sig_eff_vs_bkg_eff(
    ...
    for legend in os.listdir(fit_dir):
        ...
        palette_dict = {
            ...
            "pretrain(ab1)": custom_colors[3],
            "pretrain(ab4)": custom_colors[4],
        ...
        for mass, legend_dict in sorted(roc_summaries.items(), key=lambda x: float(x[0])):
            ...
            for base_legend, (xs, vals, uncs) in sorted(legend_data.items()):
                ...
                if 'spanet' in base_legend:
                    ...
                    if 'ablation1' in base_legend:
                        model_type = "pretrain(ab1)"
                    elif 'ablation4' in base_legend:
                        model_type = "pretrain(ab4)"
                    elif 'pretrainv1' in base_legend:
                        model_type = "pretrain"
                    else:
                        continue
                else:
                ...
                if not (model_name == "spanet"):
                    if pretrain_on:
                        if 'ab1' in model_type:
                            legend_name += "-f.t.(ab1)"
                        elif 'ab4' in model_type:
                            legend_name += "-f.t.(ab4)"
                        else:
                            legend_name += "-f.t."
                    else:
                ...
                if assignment_on and not segmentation_on:
                    ...
                    linewidth = 1
                    coloralpha = 0.5
                    custom_lines["w Cls+Assign"] = Line2D([0], [0], color='black', marker=markerstyle,
                                                          linestyle=linestyle)
                    ...
                    linewidth = 1
                    coloralpha = 0.5
                    custom_lines["w Cls+Seg"] = Line2D([0], [0], color='black', marker=markerstyle, linestyle=linestyle)
                    ...
                    linewidth = 1
                    coloralpha = 0.5
                    custom_lines["w Cls+Assign+Seg"] = Line2D([0], [0], color='black', marker=markerstyle,
                                                              linestyle=linestyle)
                    ...
                    linewidth = 2
                    coloralpha = 0.75
                    custom_lines["w Cls"] = Line2D([0], [0], color='black', marker=markerstyle, linestyle=linestyle)

                legend_name = None  # if not((model_type == "other") or (not assignment_on and not segmentation_on)) else legend_name

                # --- Main plot ---
                ax_main.plot(xs, vals, label=legend_name, color=color,
                             linestyle=linestyle, marker=markerstyle, linewidth=linewidth, alpha=coloralpha)
                if any(uncs):
                    ...
                    #                    else:
                    if not assignment_on and not segmentation_on:
                        ax_main.fill_between(xs, lower, upper, color=color, alpha=0.2)
                ...
                ax_ratio.plot(xs, ratios, color=color, linestyle=linestyle, marker=markerstyle, linewidth=linewidth,
                              alpha=coloralpha)
                if any(ratio_uncs):
                    ...
                    #                    else:
                    if not assignment_on and not segmentation_on:
                        ax_ratio.fill_between(xs, lower, upper, color=color, alpha=0.2)
            ...
            custom_lines_model = [
                Line2D([0], [0], color=palette_dict['spanet'], marker='o', linestyle='None'),
                Line2D([0], [0], color=palette_dict['other'], marker='o', linestyle='None'),
                Line2D([0], [0], color=palette_dict['pretrain'], marker='o', linestyle='None'),
                Line2D([0], [0], color=palette_dict['pretrain(ab1)'], marker='o', linestyle='None'),
                Line2D([0], [0], color=palette_dict['pretrain(ab4)'], marker='o', linestyle='None')
            ]

            # Add legend
            title_font = font_manager.FontProperties(weight='bold', size=12)
            legend1 = ax_main.legend(custom_lines_model,
                                     ['SpaNet', 'EveNet-scratch', 'EveNet-f.t.(Cls+Seg+Gen)', 'EveNet-f.t.(SSL)',
                                      'EveNet-f.t.(Cls+Gen)'], title="model", loc='upper left',
                                     title_fontproperties=title_font)
            ax_main.add_artist(legend1)
            legend2 = ax_main.legend(custom_lines.values(), custom_lines.keys(), title="training", loc='upper right',
                                     title_fontproperties=title_font)
            ...
            ymax = max(ax_main.get_ylim())  # get current upper limit
            ax_main.set_ylim(5e-1, ymax * 100)
...
def convert_metrics_key(key, mass):
    """
    Convert metric key by replacing MASS with actual mass value.

    Args:
        key (str): Metric key with "MASS" placeholder.
        mass (str): Actual mass value to replace "MASS".

    Returns:
        str: Converted metric key.
    """
    word_dict = {
        30: "thirty",
        40: "forty",
...
def plot_metric_vs_dataset_size(
    ...
    for legend in os.listdir(fit_dir):
        if "Purity" in metric_key:
            assignment_results = os.path.join(fit_dir, legend, "summary.json")
            if not os.path.isfile(assignment_results):
                print(f"Warning: assignment results not found for {legend}, skipping.")
                continue
            with open(assignment_results, 'r') as f:
                roc_data = json.load(f)

        else:
            roc_results = os.path.join(fit_dir, legend, "summary", "roc_results.npz")
            if not os.path.isfile(roc_results):
                print(f"Warning: roc_results not found for {legend}, skipping.")
                continue
            roc_data = np.load(roc_results, allow_pickle=True)

        base_legend, dataset_size = parse_legend(legend)

        if float(dataset_size) < 0.02:
            continue

        for m in masses:
            key = f"haa_ma{m}"
            if key not in roc_data:
                print(f"Warning: mass {m} not found in roc_data for {legend}, skipping. {roc_data.keys()}")
                continue
            try:
                if "Purity" in metric_key:
                    value = roc_data[key][convert_metrics_key(metric_key, m)]
                else:
                    sr_data = roc_data[key].item()["SR"]
                    value = extract_metric(sr_data, metric_key)
                ...
                if base_legend not in roc_summaries[m]:
                    roc_summaries[m][base_legend] = []
                roc_summaries[m][base_legend].append(
                    (float(dataset_size) * training_dataset_size_absolute, value, value_unc if unc_metrics else None))
            except Exception as e:
                print(f"Error processing {key} in {legend}: {e}")
    ...
    with open(os.path.join(output_dir, f"{filename_prefix}_summaries_{metric_key.replace('/', '.')}.json"), "w") as f:
        json.dump({k: {lk: v for lk, v in lv.items()} for k, lv in roc_summaries.items()}, f, indent=4)
    ...
    for mass, legend_dict in sorted(roc_summaries.items(), key=lambda x: float(x[0])):
        ...
        for base_legend, (xs, vals, uncs) in sorted(legend_data.items()):
            ...
            # # TODO: remove
            # if "pretrain" not in base_legend:
            #     continue
            ...
            # Linestyle by assignment flag
            is_solid = 'assignment-on' in base_legend
            alpha_fill = 0.2 if is_solid else 0.2
            hatch = None if is_solid else '///'
            ...
            if not (model_name == "spanet"):
                if pretrain_on:
                    if 'ab1' in model_type:
                        ...
                        legend_name += "-f.t.(Cls + Gen)"
                    else:
                        legend_name += "-f.t.(Cls + Seg + Gen)"
                else:
                    legend_name += "-Scratch"
            ...
            if assignment_on and not segmentation_on:
                ...
                linestyle = "--"
                markerstyle = "+"
                linewidth = 1 if "Purity" not in metric_key else 2
                coloralpha = 0.5
                custom_lines["w Cls+Assign"] = Line2D([0], [0], color='black', marker=markerstyle, linestyle=linestyle)
        ...
        ymax = max(ax_main.get_ylim())  # get current upper limit
        ax_main.set_ylim(0, ymax * 1.5)  # extend top by factor 1.5
...
def main():
    ...
    fit_ass_dir = os.path.join(args.store_dir, "assignment_metrics")
